...
if (!$_POST['data'] == "") {
    ...
    $strNational = preg_replace("'<li>.*?.<a'", "<li><a", $strNational[0][count($strNational[0]) - 1]); //BUGFIX

    preg_match("'<h3>.*?Профессиональные.*?</h3>.*?</ul>'", $str, $strProfessional);
    preg_match_all("'<ul>.*?</ul>'", $strProfessional[0], $strProfessional);
    $strProfessional = preg_replace("'<li>.*?.<a'", "<li><a", $strProfessional[0][count($strProfessional[0]) - 1]); //BUGFIX

    preg_match("'<h3>.*?Международные.*?</h3>.*?</ul>'", $str, $strInternational);
    preg_match_all("'<ul>.*?</ul>'", $strInternational[0], $strInternational);
    $strInternational = preg_replace("'<li>.*?.<a'", "<li><a", $strInternational[0][count($strInternational[0]) - 1]); //BUGFIX
